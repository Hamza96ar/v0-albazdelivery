...
export default function DriverApp() {
  ...
  const fetchActiveDelivery = async () => {
    try {
      const response = await fetch(`/api/drivers/deliveries?driverId=${driverId}`)
      const data = await response.json()
      if (data.success) {
        const active = data.deliveries.find((d: Order) => d.status === "assigned" || d.status === "in_delivery")
        setActiveDelivery(active || null)

        // Get completed deliveries for history
        const completed = data.deliveries.filter((d: Order) => d.status === "delivered")
        setDeliveryHistory(completed)
      }
    } catch (error) {
      console.error("[v0] Error fetching active delivery:", error)
    }
  }
  ...
  const acceptDelivery = async (orderId: string) => {
    try {
      const response = await fetch("/api/drivers/deliveries/accept", {
        method: "POST",
